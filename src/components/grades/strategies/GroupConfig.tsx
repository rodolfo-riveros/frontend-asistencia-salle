...
export function GroupConfig({ students, groupSize, setGroupSize, studentGroups, setStudentGroups, newColName }: GroupConfigProps) {
  ...
  return (
    <div className="space-y-6 p-8 bg-primary/5 rounded-[2.5rem] border-2 border-primary/10 border-dashed">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-1">
          <h4 className="text-xl font-black text-primary uppercase tracking-tighter flex items-center gap-2"><Users className="h-5 w-5" /> Configuración de Equipos</h4>
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Mezcla aleatoria o asignación manual de estudiantes</p>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <Button className="bg-primary hover:bg-primary/90 h-14 px-8 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-primary/20 text-white" onClick={generateRandomGroups}>Sorteo Aleatorio</Button>
          </div>
          ...
          <Button 
            variant="outline" 
            size="icon" 
            className="h-14 w-14 rounded-2xl border-2 border-primary/10 text-primary hover:bg-primary/5 shadow-sm shrink-0" 
            onClick={exportGroupsToPdf}
            title="Exportar conformación de equipos en PDF"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {students.map(s => (
          <div key={s.id} className="bg-white p-4 rounded-2xl border-2 border-slate-100 shadow-sm flex flex-col gap-3 group hover:border-primary/20 transition-all">
            ...
            <select 
              ...
              className="w-full h-9 text-[9px] font-black uppercase tracking-widest border-2 rounded-lg bg-slate-50 border-slate-100 outline-none focus:border-primary/30 transition-colors"
            >
