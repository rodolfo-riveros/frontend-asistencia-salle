...
export default function InstructorQuizPage() {
  ...
  const handleCloseAndPersist = async () => {
    ...
    try {
      ...
      let successCount = 0;
      // MOTOR DE GUARDADO SECUENCIAL CON RETARDO (SOLUCIÓN PARA GRUPOS GRANDES 32/60+)
  ...
  const handleExportQuestions = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(18); doc.setTextColor(0, 51, 102); doc.text("IES LA SALLE URUBAMBA", 14, 20);
    doc.setFontSize(12); doc.setTextColor(100); doc.text(`BANCO DE PREGUNTAS TÉCNICAS: ${config.nombre.toUpperCase()}`, 14, 30);
    const questions = config.configuracion_json.questions;
    ...
    autoTable(doc, {
      startY: 40, head: [['N°', 'PREGUNTA', 'ALTERNATIVAS']], body: body, theme: 'grid',
      headStyles: { fillColor: [0, 51, 102], textColor: 255 }, styles: { fontSize: 8 }
    });
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="icon" onClick={handleExportQuestions} className="h-10 w-10 border-red-200 text-red-600 hover:bg-red-50 rounded-xl transition-all shadow-sm">
                    <Download className="h-5 w-5" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent className="bg-red-600 text-white font-black text-[10px] uppercase tracking-widest">Banco PDF</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          ...
          {!roomCode ? (
            ...
            <div className="flex items-center gap-6 bg-white border-2 border-slate-100 p-6 rounded-[2.5rem] shadow-2xl">
              ...
              <Button onClick={() => setIsFullscreen(true)} className="bg-primary text-white h-20 px-12 rounded-[2rem] font-black uppercase text-sm tracking-widest gap-4 shadow-xl hover:scale-105 active:scale-95 transition-all">
                <Maximize2 className="h-6 w-6" /> PROYECTAR ARENA
              </Button>
            </div>
          )}
        </div>
